...
- Idle and active measurements are scheduled about every 100ms. The first valid <200mm reading immediately creates a RAM-only provisional visit anchored to that first blockage and starts an RFID scan. Valid >=200mm is clear. An unidentified provisional visit is neither persisted nor uploaded.
...
- Each RFID scan lasts at most ten seconds. A registered chip is accepted immediately; a complete unregistered chip is treated as interference and ends that scan immediately without cancelling the distance-based provisional visit. If entry identification is missed, a registered exit reading can backfill the visit identity.
...
- One registered ID on either side identifies the visit; matching IDs on both sides confirm it. Two different registered IDs create a sticky conflict and discard the visit. No identity on either side also discards it. Discarded visits are never persisted or uploaded, and no `unknown` visit is created.
- No exit candidate within five minutes of the first blockage closes with `no_exit_timeout`. A candidate started before five minutes may finish through 5m10s; otherwise it closes with `exit_unconfirmed_timeout`. Identified timeout visits keep their identity; unidentified ones are discarded.
...
The event start never resets during scans or phase changes. Duration and inferred exit time use the first exit candidate, or entry plus five minutes if none exists; finishing and network delays are excluded. Without reliable UTC at entry, date fields remain blank and are never filled with upload time.
...
After a registered identity is obtained, the active visit is checkpointed immediately and every 30 seconds; unidentified provisional visits remain RAM-only. After reboot, old active journals are kept locally as interrupted records, never automatically uploaded or given a fresh five minutes. New events wait for entrance recovery. There is one interrupted slot; a full slot or failed save/removal preserves existing data, disables active journaling and reports an error. Completed pending records remain independent. Diagnostic error counters live in RAM for this boot, not a permanent audit log.
...
- 待機與活動皆約每 100ms 量測。第一次有效距離 <200mm 立即以首次遮擋時間建立只存在 RAM 的暫定事件並掃描 RFID；有效 >=200mm 為清空。暫定事件沒有身分時不保存、不上傳。
- 入口活動中的反覆遮擋合併為同一事件。有效距離持續 >=200mm 滿 10 秒才轉為「推定在內部」，不結案。相鄰有效清空樣本相隔不超過 1 秒時，依實際經過時間累積；超過 1 秒視為取樣中斷，當前樣本重新開始計時。距離無效或再次遮擋也歸零。
- 每輪 RFID 掃描最多 10 秒。已登錄晶片立即接受；完整但未登錄的晶片視為干擾並立即停止該輪，但不取消距離暫定事件。入口沒有取得身分時，事件仍可在離開掃描取得已登錄晶片後回填。
- 推定在內部之後，下一次遮擋建立離開候選並再次掃 RFID。只有離開後連續清空 10 秒、且該輪掃描已完成，才正常結案。離開階段反覆活動不重啟掃描。
- 入口與離開只有一側取得已登錄 ID 時，使用該 ID；兩側取得相同 ID 時正常確認；取得兩個不同的已登錄 ID 時鎖定衝突並整筆作廢；兩側都沒有身分也作廢。作廢事件不保存、不上傳，系統不建立 `unknown` 如廁紀錄。
- 從第一次遮擋滿 5 分鐘仍無離開候選，以 `no_exit_timeout` 結案。5 分鐘前已有候選，可收尾至第 5 分 10 秒；屆時尚未符合正常條件，使用 `exit_unconfirmed_timeout`。已辨識事件依原身分保存；未辨識事件作廢。
- 正常結案後立即接受新遮擋，不另設冷卻。強制結案時入口仍遮擋、無效或資訊過期，必須先看到有效清空，再接受新遮擋。

事件起點不因掃描、清空或階段改變而重設。有離開候選時，停留時間與推定離開時間採首次候選時間；無候選則採起點加 5 分鐘。不計入收尾及網路等待。起點沒有可靠 UTC 時，日期欄留空，不以上傳時間回填。
...
取得已登錄身分後，活動事件立即備份並每 30 秒更新；尚未辨識的暫定事件只存在 RAM。重啟後舊活動備份只保留為本機中斷紀錄，不自動上傳、不重新給 5 分鐘；入口恢復後才接受新事件。中斷槽只有一筆，已滿或保存／刪除失敗時保留原資料並停用活動備份，顯示錯誤；已完成的待傳佇列仍獨立處理。錯誤計數為本次開機的 RAM 診斷值，不是永久稽核紀錄。
