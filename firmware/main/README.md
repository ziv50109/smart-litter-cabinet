...
停留時間是第一次遮擋到建立離開候選之第二次遮擋的差值，不包含 RFID 或網路等待。只有 `normal_exit`、已辨識到本機登錄貓咪，且事件中沒有讀到互相衝突的身分時才會建立 Sheet 紀錄；其他結果僅保留診斷資料。
...
- STA 可連上網際網路時，維護期間會嘗試上傳 NVS 待傳紀錄
...
OTA 使用的分割區配置必須同時包含 `otadata`、`ota_0`、`ota_1` 與 SPIFFS；不要把 bootloader、partition image 或 merged image 當成應用程式映像檔上傳。
