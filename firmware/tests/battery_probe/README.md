...
測距門檻、測距週期、RFID 掃描逾時、觸發後記錄時間與等待觸發期限皆定義在 `probe_logic.h`：
...
README 不重複這些參數的目前數值。硬體接腳以 `battery_probe.ino` 與 `hardware/README.md` 為準。

## 測試模式

| 指令 | 模式 | RFID／睡眠 | 結束條件 |
|---|---|---|---|
| `a` | gated + awake | RFID 按需開啟；CPU 不進 Light-sleep | 入口觸發後完成一輪記錄，或等待觸發逾時 |
| `b` | gated + Light-sleep | RFID 按需開啟；允許 Light-sleep | 同上；一般驗證優先使用此模式 |
| `c` | always-on reference | RFID 常開；CPU 不睡 | 完成一輪記錄 |
| `i` | idle sleep | RFID 按需；允許 Light-sleep | 完成一輪記錄 |
| `d` | dump | 讀取最後一次測試記錄 | — |
...
3. Serial Monitor 使用 sketch 設定的 baud rate，送 `h` 確認設定。
...
- `commanded_on_ms`：RFID 啟用接腳被命令為 HIGH 的累積時間，不等於實際電流或電量。
...
`S.flags`：bit0=有效取樣、bit1=低於 `Probe::THRESHOLD_MM`、bit2=該次取樣時 RFID 啟用接腳為 HIGH。所有時間都是本次開機 uptime，不是實際時刻。
