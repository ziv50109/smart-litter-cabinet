...
開啟前複製 `vl53l0x_distance_web/secrets.example.h` 為 `secrets.h` 並填入本機 Wi-Fi。本測試的顯示分類門檻只是調校輔助，不是主系統的進出判斷參數。
...
Before use, copy `vl53l0x_distance_web/secrets.example.h` to `secrets.h` and set the local Wi-Fi credentials. The displayed distance categories are calibration aids, not the main system's entry/exit thresholds.
